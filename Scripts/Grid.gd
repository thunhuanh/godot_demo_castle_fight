...
func _draw():
	if on:
		...
		for i in range(int((cam.x - size.x) / gridSize) - 1, int((size.x + cam.x) / gridSize) + 1):
			draw_line(Vector2(i * gridSize, cam.y + size.y + 100), Vector2(i * gridSize, cam.y - size.y - 100), "535050")
		for i in range(int((cam.y - size.y) / gridSize) - 1, int((size.y + cam.y) / gridSize) + 1):
			draw_line(Vector2(cam.x + size.x + 100, i * gridSize), Vector2(cam.x - size.x - 100, i * gridSize), "535050")
